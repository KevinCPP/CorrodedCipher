...
class UI_Element_Factory:
    ...
    def getValue(self, label):
        ...
        if isinstance(widget, QSpinBox):
            ...
            return widget  # Return None for unsupported widget types
    
    def make_password_entry_element(self, label, tooltip=None, callback=None) -> QHBoxLayout:
        # Create a QLineEdit widget for password input
        password_box = QLineEdit()
        password_box.setEchoMode(QLineEdit.Password)  # Set the echo mode to password to obscure the text
        
        # Set the tooltip if provided
        if tooltip:
            password_box.setToolTip(tooltip)
        
        # Set the widget width if specified
        if self.widget_width:
            password_box.setFixedWidth(self.widget_width)

        # Connect the callback if provided
        if callback:
            password_box.textChanged.connect(callback)

        # Store the widget for later retrieval
        self.widget_values[label] = password_box

        # Create a label for the password box
        label_widget = QLabel(label)
        if tooltip:
            label_widget.setToolTip(tooltip)

        # Arrange the label and password box in a horizontal layout
        hbox = QHBoxLayout()
        hbox.addWidget(label_widget)
        hbox.addWidget(password_box)
    ...
    def make_directory_explorer_element(self, label, tooltip=None, initial_path=None, callback=None) -> QHBoxLayout:
        # Create a button that will open the directory dialog
        button = QPushButton("Browse...")
        ...
        def open_directory_dialog():
            options = QFileDialog.Options()
            directory_name = QFileDialog.getExistingDirectory(None, "Select Directory", "", options=options)
            if directory_name:
                line_edit.setText(directory_name)
                self.widget_values[label] = directory_name  # Store the selected directory path for later retrieval

                if callback:
                    callback(directory_name)

        # Connect the button's clicked signal to open the directory dialog
        button.clicked.connect(open_directory_dialog)
